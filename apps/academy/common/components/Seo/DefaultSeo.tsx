...
export const DefaultSeo: FC<DefaultSeo> = ({ seo }) => (
  <NextDefaultSeo
    ...
    openGraph={{
      ...
      images: [
        {
          url: seo?.defaultSeo?.shareImage?.data?.attributes?.url as string,
          width: seo?.defaultSeo?.shareImage?.data?.attributes?.width as number,
          height: seo?.defaultSeo?.shareImage?.data?.attributes?.height as number,
          type: 'image/jpeg',
